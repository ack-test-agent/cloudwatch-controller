...
// syncTags reconciles the tags on an InsightRule by calling TagResource for tags
// to add or update, and UntagResource for tags to remove.
// PutInsightRule silently ignores the Tags field on updates to existing rules,
...
func (rm *resourceManager) syncTags(
	...
	if latest.ko.Status.ACKResourceMetadata == nil || latest.ko.Status.ACKResourceMetadata.ARN == nil {
		return fmt.Errorf("cannot sync tags: InsightRule ARN is not yet available in status")
	}
