...
def exists(rule_name):
    ...
    """
    return get(rule_name) is not None
...
def get(rule_name):
    ...
    # describe_insight_rules does not support botocore paginators, so we
    # paginate manually using NextToken.
    kwargs = {}
    while True:
...
def get_tags(rule_arn):
    """Returns a list of tag dicts for the InsightRule from the CloudWatch API.

    If the resource doesn't exist, returns None.
    """
    c = boto3.client('cloudwatch')
    try:
