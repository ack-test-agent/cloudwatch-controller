...
def _make_insight_rule(name_prefix: str, resource_name: str,
    ...
    """
    rule_name = random_suffix_name(name_prefix, 24)

    replacements = REPLACEMENT_VALUES.copy()
    replacements["INSIGHT_RULE_NAME"] = rule_name
    ...
    insight_rule.wait_until_deleted(rule_name)
...
class TestInsightRule:
    def test_crud(self, _insight_rule):
        ...
        assert insight_rule.exists(rule_name)
        assert k8s.get_resource_exists(ref)

        # Verify the CR has the ARN populated
        cr = k8s.get_resource(ref)
        assert cr["status"]["ackResourceMetadata"]["arn"] is not None
        assert f"insight-rule/{rule_name}" in cr["status"]["ackResourceMetadata"]["arn"]

        # Verify the state is ENABLED
        assert cr["spec"]["state"] == "ENABLED"

        # Verify via the AWS API
        aws_rule = insight_rule.get(rule_name)
        assert aws_rule is not None
        assert aws_rule["State"] == "ENABLED"

        # Update the rule: change state to DISABLED
        updates = {
            "spec": {
                "state": "DISABLED",
            }
        ...
        # Verify the state change via the AWS API
        aws_rule = insight_rule.get(rule_name)
        assert aws_rule is not None
        assert aws_rule["State"] == "DISABLED"

        # Verify the CR reflects the change
        cr = k8s.get_resource(ref)
        assert cr["spec"]["state"] == "DISABLED"
...
class TestInsightRuleTags:
    ...
    def test_tag_sync(self, _tagged_insight_rule):
        ...
        # Verify via the AWS API.
        aws_tags = insight_rule.get_tags(rule_arn)
